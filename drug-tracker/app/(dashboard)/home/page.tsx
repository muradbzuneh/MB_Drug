...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-[#1b345f] bg-[url('/Background.jpg')] p-10">
        <h1 className="text-2xl font-bold text-cyan-500">
          Welcome{session?.user?.name ? `, ${session.user.name}` : ""}
        </h1>
        <p className="mt-1 text-[20px] text-emerald-900">Here&apos;s your health overview.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {cards.map((c) => (
          <Link
            key={c.href}
            href={c.href}
            className="group rounded-2xl border border-[#1b345f] bg-emerald-800 p-6 transition hover:border-emerald-100 hover:bg-emerald-700"
          >
